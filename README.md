...
HF filter can be bypassed using JP1 by scratching solder jumper contact 1 (which is bridged on PCB by default) and soldering contact 3. 

I'm not sure if the measurement of solar panel voltage by the ATmega will not interfere with the SPV1040 power converter function, so I placed JP2 on the board (bridged by default) which can be easily scratched to break the solar panel - AD converter input connection.
...
The board will be produced in [LJCPCB](https://jlcpcb.com/) and I will add here the link to order it.

The board was not yet produced nor tested! 

Jakub Šerých, serych@panska.cz, [https://jakub.serych.cz](https://jakub.serych.cz) (only in Czech language)
